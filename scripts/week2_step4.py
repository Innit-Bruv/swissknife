...
# ── Load draft model (Llama 1B — more stable than Qwen 0.5B) ─────────────────
...
draft_model.eval()
print(f"Draft model loaded. VRAM: {torch.cuda.memory_allocated() / 1e9:.2f} GB")
...
def score_safety(context_text, candidate_text):
    ...
    return probs[0].item()  # probability of being non-hate

# ── Core generation function ──────────────────────────────────────────────────
def generate_with_blade(prompt, score_fn, max_new_tokens=80, K=4):
...
for prompt in prompts:
    ...
    print("\n[BLADE 1 - helpfulness]")
    print(generate_with_blade(prompt, score_helpfulness))

    print("\n[BLADE 2 - safety]")
    print(generate_with_blade(prompt, score_safety))
